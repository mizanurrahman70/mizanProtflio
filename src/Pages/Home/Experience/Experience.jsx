  ...
  return (
    <div className="my-20 px-4 max-w-5xl mx-auto">
      ...
      {experiences.map((experience) => (
        <div
          key={experience.id}
          className="bg-white rounded-xl shadow-lg p-8 border border-gray-200 mb-8 hover:shadow-2xl transition duration-300"
        >
          ...
          <div className="flex flex-col md:flex-row justify-between mb-4">
            ...
            <div className="text-sm text-gray-500">
              ...
              {experience.endDate && (
                <span> - End Date: {experience.endDate}</span>
              )}
            </div>
          </div>
          ...
          <ul className="list-disc list-inside space-y-2 text-gray-700 text-sm md:text-base">
            {experience.responsibilities?.map((responsibility, index) => (
              <li key={index} className="hover:text-green-600 transition duration-200">
                {responsibility}
              </li>
            ))}
          </ul>

         {experience.role &&( <p className="mt-4 font-semibold text-gray-700">
            Role:{" "}
            <span className="font-normal text-gray-500">{experience.role}</span>
          </p>)}
         {experience.certificate &&( <p className="mt-4 font-semibold text-gray-700">
          certificate:{" "}
            <span className="font-normal text-gray-500">{experience.certificate}</span>
          </p>)}
